...
def solve_ivp():
    ...
    with cols[0]:
        ...
        discharge = st.number_input(
            "Discharge -- $Q$ [m³/s]", 0.1, 100.0, 12.5, 1.0, format="%.1f"
        )
        n_manning = st.number_input(
            "Mannning coef. -- $n$ [-]", 0.010, 0.070, 0.025, 0.001, format="%.3f"
        )
        ...
        side_slope = st.number_input(
            "Side slope -- $m$ [-]", 0.0, 10.0, 1.0, 0.1, format="%.4f"
        )
        y0 = st.number_input(
            "Boundary condition -- $y(x_0)$ [m]", 0.01, 10.0, 2.0, 0.1, format="%.2f"
        )

    with cols[1]:
        lilcols = st.columns(2, vertical_alignment="center")

        with lilcols[0]:
            st.markdown("##### Normal depth calculation")

        with lilcols[1]:
            normaldepth_container = st.empty()
        ...
        with lilcols[0]:
            st.markdown("##### Critical depth calculation")
            
        with lilcols[1]:
            critdepth_container = st.empty()
    ...
    if water_surface.success:
        st.pyplot(
            plot_water_profile(
                x, y, normal_depth.x[0], critical_depth.x[0], bottom_slope
            )
        )
    ...
    with col:
        st.info("### 🤔 What type of profile is this?")


def plot_water_profile(x, y, y_n, y_c, S0):
    fig, ax = plt.subplots()
