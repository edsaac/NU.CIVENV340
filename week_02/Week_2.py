...
if option == "Friction head loss":
    ...
    *****
    ...
    cols = st.columns(3)
    with cols[0]: f_dw = st.slider("Darcy-Weisbach -- $f$", 0.008, 0.1, 0.030, step=0.001, key="dw_f", format="%.3f")
    with cols[1]: c_hw = st.slider(r"Hazen-Williams -- $C_{\texttt{HW}}$", 75, 160, 110, step=1, key="c_hw", format="%d")
    with cols[2]: n_man = st.slider(r"Manning -- $n$", 0.009, 0.030, 0.010, step=0.001, key="n_man", format="%.3f")

    cols = st.columns(2)
    with cols[0]: length = st.number_input("Pipe lenght -- $L$ [m]", 1, 100, 10, key="len_pipe", format="%d")
    with cols[1]: diameter = st.number_input("Pipe diameter -- $D$ [m]", 0.005, 0.25, 0.050, key="diam_pipe", format="%.3f")
    ...
    hovertemplate = "Q = %{x:.1f} m³/s <br><b>h<sub>f</sub> = %{y:.2f} m</b>"
    fig = go.Figure([
        go.Scatter(
            x=discharge, 
            y=hf_dw,
