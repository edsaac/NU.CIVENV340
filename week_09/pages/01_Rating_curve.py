...
def main():
    ...
    "# Rating curves"
    img_url = "assets/img/RatingCurve_02215500.png"
    ...
    with st.sidebar:
        site_id = st.selectbox("Site number", ["02215500"])
    ...
    st.markdown("""
        <iframe src="https://www.google.com/maps/embed?pb=!4v1684686947687!6m8!1m7!1spJqaWeiIkdWa9jYr_jRi6g!2m2!1d31.9199824386102!2d-82.67446321198821!3f236.12162750392207!4f-5.393784221498308!5f0.7820865974627469" width="800" height="600" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
        """, 
        unsafe_allow_html=True
    )

    "&nbsp;"
    url = "https://waterdata.usgs.gov/monitoring-location/02215500/#parameterCode=00065&period=P7D"
    st.button('Check it out at `waterdata.usgs.gov`', on_click=open_page, args=(url,), use_container_width=True)

    "************"
    "## Using USGS `dataretrieval`"
    ...
    "************"
    "## Getting the field measurements"

    start, end = st.date_input("Date range", [date(2010,1,1), date(2023,5,1)], date(1950,1,1), date(2023,5,21))
    with st.echo():    
    ...
    "************"
    r"## Fitting an equation"
    """
    $$
        \log{Q} \propto H \implies Q = a \exp(b\,H) + c
    $$
    ...
    cols = st.columns(3)
    with cols[0]: st.metric("$a$", f"{a:.2E}")
    with cols[1]: st.metric("$b$", f"{b:.2E}")
