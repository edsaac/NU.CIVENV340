...
def main():
    ...
    for node in gv_network["graph"]["nodes"].values():
        ...
        node["metadata"]["click"] = fr" X = {x:.2f} m<br> Y = {y:2f} m<br> Demand = {q:.2e} m³/s"
    ...
    if submit_btn:
        ...
        ****
        ...
        for edge, edge_df in zip(edges, edges_df.itertuples()):
            ...
            edge["metadata"]["click"] = fr" Length = {edge_df.Length:.2f} m<br> Diameter = {edge_df.Diameter:2f} m<br> Roughness = {edge_df.Roughness:.2e} m"
            edge["metadata"]["hover"] = f"{edge_df.Index}: {edge_df.Source} -- {edge_df.Target}"
        ...
        for node, node_df in zip(nodes.values(), nodes_df.itertuples()):
            ...
            node["metadata"]["click"] = fr" X = {node_df.X:.2f} m<br> Y = {node_df.Y:2f} m<br><b> Demand = {node_df.Demand:.1f} m³/s</b>"
        ...
        ****
        ...
        if solved_Q.success:
            solved_edges_df = edges_df[["Source", "Target", "Length", "Diameter", "Roughness"]]
            
            solved_edges_df["Q_Solved"] = solved_Q.x
            solved_edges_df["Re"] = 4.0 * np.abs(solved_edges_df["Q_Solved"]) / (np.pi * solved_edges_df["Diameter"] * ν)
            solved_edges_df["e/D"] = solved_edges_df["Roughness"] / solved_edges_df["Diameter"]
            solved_edges_df["f"] = swamme_jain(solved_edges_df["e/D"], solved_edges_df["Re"])
            ...
            ****
            ...
            for edge, discharge in zip(edges, solved_Q.x):
                ...
                edge["metadata"]["click"] = edge["metadata"]["click"] + f"<br><b> Discharge = {discharge:.3f} m³/s</b>"
